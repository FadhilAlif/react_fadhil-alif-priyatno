...
import { addProductData, getProducts } from "../../api";
...
const DetailProduct = ({ setProductsTable }) => {
  ...
  const fetchData = () => {
    getProducts()
      .then((respone) => {
        setProductsTable(respone.data);
        console.log("successfully save data =>", respone.data);
      })
      .catch((error) => {
  ...
  const handleSubmit = (e) => {
    ...
    if (Object.keys(error).length === 0) {
      ...
      addProductData(data)
        ...
        .catch((error) => {
          console.log("error add product to API=> ", error);
        });
